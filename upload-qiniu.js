...
const baseUrl = 'http://qiniucdn.imfdj.top/';
...
(async function () {
  try {
    await readDir(path.resolve(__dirname, './dist/static'));
    ...
    // fs.writeFileSync(indexPath, indexHtml.replace(RegExp(`static/js/${ chunkLibsFilesJs }`, "g"), `${ baseUrl }${ prefix }${ chunkLibsFilesJs }`))
    // console.log('indexHtml-replace-done!!!');
  } catch (e) {
    throw e;
  }
})();
